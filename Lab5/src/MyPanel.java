...
public class MyPanel extends JPanel {
	...
	private static final int mine_COUNT = 13;
	private Color[][] colorArray = new Color[TOTAL_COLUMNS][TOTAL_ROWS];
	private boolean[][] mineArray = new boolean[TOTAL_COLUMNS][TOTAL_ROWS];
	...
	public MyPanel() {
		...
		this.setUp(); //Runs tile and mine setup method
	}
	...
	public void paintComponent(Graphics g) {	//Method in charge of painting tiles and numbers
		...
		for (int x = 0; x < TOTAL_COLUMNS; x++) {
			for (int y = 0; y < TOTAL_ROWS; y++) {
				...
				//If the tile has been pressed, count surrounding mines
				if (this.getColor(x, y) == Color.GRAY) {
					int mineCount = this.surroundingMines(x, y);
					//Call the method to paint the number
					this.paintNumber(g, mineCount, x1 + GRID_X + (x * (INNER_CELL_SIZE + 1)) + 10, y1 + GRID_Y + (y * (INNER_CELL_SIZE + 1)) + 22);
				}
			}
		}	
	}
	public void setUp() {	//This method paints the tiles and sets the mine locations, used to reset board
		flagCounter = 0; //Set the flag count to 0
		
		for (int x = 0; x < TOTAL_COLUMNS; x++) {   //Delete any existing mines
			for (int y = 0; y < TOTAL_ROWS; y++) {
				mineArray[x][y] = false;
			}
		}
		...
		for (int x = 0; x < mine_COUNT;) {		//Assign mines to random spaces
			int i = new Random().nextInt(TOTAL_COLUMNS);
			int j = new Random().nextInt(TOTAL_ROWS);
			if(mineArray[i][j] == false) { 		//Assigns a mine if the space doesn't have one already
				mineArray[i][j] = true;
				x++;
			}
		}
	...
	public int getMines() {		//Returns the number of mines
		return mine_COUNT;
	}
	public boolean isMine(int x, int y) {	//Method to verify if the selected square is a mine
	...
	public void paintNumber(Graphics g, int mineCount, int x, int y) {		//Method to paint each number with a different color and display it
		switch(mineCount){
		case 0:
			break;
		case 1:
		...
		g.drawString(mineCount + "" + "", x, y);
	}
	public int surroundingMines(int x, int y) {		//Method for counting number of mines around a tile.
	...
	public void chain(int x, int y) { 	//Recursive method to uncover a chain of tiles that have no surrounding mines
		//Check if there aren't any surrounding mines
		if(this.surroundingMines(x, y) == 0) {
			for(int i=x-1; i<=x+1; i++) {
				for (int j=y-1; j<=y+1; j++) {
					//Check if coordinates are inside the grid
					if(i < this.getColumns() && i >= 0 && j < this.getRows() && j >= 0) {
	...
	public void minePressed() {		//Method for when user loses the game. Uncovers all tiles
	...
	public void winConditions(){	//Check if the player has won
		for(int i = 0; i < this.getColumns(); i++) {
			for(int j = 0; j < this.getRows(); j++) {
				if((this.getColor(i,j) == Color.WHITE || this.getColor(i,j) == Color.RED) && this.isMine(i,j) == false){
					//If there are still covered tiles that are not mines, game hasn't ended.
					return;
				}
			}
		}
